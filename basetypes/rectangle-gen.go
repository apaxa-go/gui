...
func (r RectangleF32) Align(size PointF32, how Align) RectangleF32 {
	var res RectangleF32
	res.Left, res.Right = alignF32(r.Left, r.Right, size.X, how.Hor())
	res.Top, res.Bottom = alignF32(r.Top, r.Bottom, size.Y, AlignHor(how.Ver()))
	return res
}

func (r RectangleF32S) Align(size PointF32, how Align) RectangleF32S {
	var res RectangleF32S
	res.Origin.X, res.Size.X = alignF32S(r.Origin.X, r.Size.X, size.X, how.Hor())
	res.Origin.Y, res.Size.Y = alignF32S(r.Origin.Y, r.Size.Y, size.Y, AlignHor(how.Ver()))
	return res
}

// It is possible to pas AlignVer as AlignHor.
func alignF32(p0, p1, size float32, how AlignHor) (r0, r1 float32) {
	switch how {
	case AlignHorLeft:
		return p0, p0 + size
	case AlignHorRight:
...
// It is possible to pas AlignVer as AlignHor.
func alignF32S(p, w, size float32, how AlignHor) (rp, rw float32) {
	switch how {
	case AlignHorLeft:
		return p, size
	case AlignHorRight:
